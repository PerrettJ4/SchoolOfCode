...
const api_url = "https://nutflix-app.herokuapp.com/films";
...
function createTile(film, gridNum) {
  console.log(film);
  let container = document.createElement("div");
  container.classList.add(`container`);
  // container.classList.add(`${String.fromCharCode(gridNum)}`);
  //
  ...
  actors.innerText = `Cast
   ${nuttify(squirrelfy(film.actors))}`;
  //
  front.append(img, title);
  back.append(titleCopy, yearGenre, rating, desc, actors);
  card.append(front, back);
  container.append(card);
  document.body.append(container);
  console.log(gridNum, String.fromCharCode(gridNum));
  container.style.gridArea = `${String.fromCharCode(gridNum)}`;
}
let gridNum = 97;
async function populateAllTiles() {
  const films = await getData();
  for (let film of films) {
    createTile(film, gridNum);
    gridNum++;
  }
}
